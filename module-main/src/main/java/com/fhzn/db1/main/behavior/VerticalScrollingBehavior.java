    ...
    extends CoordinatorLayout.Behavior<V>
{
    ...
    @IntDef({ScrollDirection.SCROLL_DIRECTION_UP,
     ...
     * @param child
     * @param direction Direction of the overscroll: SCROLL_DIRECTION_UP,
     *            SCROLL_DIRECTION_DOWN
     * @param currentOverScroll Unconsumed value, negative or positive based on
     *            the direction;
     * @param totalOverScroll Cumulative value for current direction
     */
    public abstract void onNestedVerticalOverScroll(
        CoordinatorLayout coordinatorLayout, V child,
        @ScrollDirection int direction, int currentOverScroll,
        int totalOverScroll);
    
    /**
     * @param scrollDirection Direction of the overscroll: SCROLL_DIRECTION_UP,
     *            SCROLL_DIRECTION_DOWN
     */
    public abstract void onDirectionNestedPreScroll(
        CoordinatorLayout coordinatorLayout, V child, View target, int dx,
        ...
        int dyUnconsumed)
    {
        ...
        onNestedVerticalOverScroll(coordinatorLayout,
            child,
        ...
        View target, int dx, int dy, int[] consumed)
    {
        ...
        mTotalDy += dy;
        onDirectionNestedPreScroll(coordinatorLayout,
            child,
            target,
            dx,
            dy,
    ...
    @Override
    public boolean onNestedFling(CoordinatorLayout coordinatorLayout, V child,
        View target, float velocityX, float velocityY, boolean consumed)
    {
        super.onNestedFling(coordinatorLayout,
            child,
            target,
            velocityX,
            velocityY,
            consumed);
        mScrollDirection = velocityY > 0 ? ScrollDirection.SCROLL_DIRECTION_UP
            : ScrollDirection.SCROLL_DIRECTION_DOWN;
        return onNestedDirectionFling(coordinatorLayout,
            child,
            target,
            velocityX,
            velocityY,
    ...
    {
        return super.onNestedPreFling(coordinatorLayout,
            child,
            target,
            velocityX,
            velocityY);
